...
module Languages
    ...
    !function templateLoader(name) result(string)
    !    character(:), allocatable :: name, string, path
    !
...
module compiler
        ...
        if (trim(command) == "") then
            ...
            if (command == "deliminator") then
                ...
                if (startswith(args, "color")) then
                    ...
                    tempList=splitByCommmas(getSecondPartAfterEQ(args))
                ...
                if (bannerTemplateChanged .EQV. .TRUE.) then
                    ...
                    do num = 1, size(tempList), 1
                        ...
                        if (startswith(temp, "image") .EQV. .TRUE.) then
                            ...
                            temp2 = tempList2(2)
                            tempList3 = splitByCommmas(temp2)
                            ...
                            do num3 = 2, size(tempList3), 1
                                replacer = replacer//animPartMaker(animPart, intToChar(num2), tempList3(num3), "3   ", "0.25")
                                replacer = replacer//animPartMaker(animPart, intToChar(num2+move), tempList3(num3), "0  ", "1   ")
                                num2 = num2+still
                                replacer = replacer//animPartMaker(animPart, intToChar(num2-move), tempList3(num3), "0   ", "1   ")
                                replacer = replacer//animPartMaker(animPart, intToChar(num2), tempList3(num3), "3   ", "0.25")
                                num2 = num2+move
                            end do
                            replacer = replacer//animPartMaker(animPart, intToChar(100), tempList3(2), "3   ", "0.25")
                ...
                call appendMainBody(tableTemplate)
                ...
                do num2 =1, size(tempList), 1
                    ...
                    if (startswith(temp, "id") .EQV. .TRUE.) then
                        ...
                        if (trim(adjustl(tempList3(2))) == "auto") then
                            ...
                            tempList2 = splitByCommmas(temp2)
                ...
                call appendMainBody(rowTemplate)
    ...
    function getSecondPartAfterEQ(string) result (part)
        ...
        if (string(x1:x1)==achar(34) .AND. string(len(string):len(string))==achar(34)) then
    ...
    function subMerge(array) result (mergedArray)
        ...
        character(:), allocatable :: temp, tempString
        ...
        do num=1, size(array), 1
            ...
            do num2=1, len(temp), 1
                if (temp(num2:num2) == achar(34)) then
    ...
    subroutine appendMainBody(string)
...
module fresh
    ...
    subroutine hopeThereWontBeAnyMemoryAllocationErrors(palette, author, title, charset, keywords,&
        ...
        if (palette /="") then
            do num = 1, 4, 1
                tempList = splitByCommmas(ColorsList%get(palette))
                temp = trim(adjustl(tempList(num)))
        ...
        !write(*,*) compiled(1:getTheLastChar(compiled))

        call removeGarbageOnceAndForAll(compiled)
    ...
    subroutine removeGarbageOnceAndForAll(text)
        character(:), allocatable, intent(inout) :: text
        integer :: num

        do num=1, len(text), 1
            if ((ichar(text(num:num))<32) .AND. &
                &ichar(text(num:num))/=9 .AND. &
                &ichar(text(num:num))/=10 .AND. &
                &ichar(text(num:num))/=13) text(num:num) = " "
        end do
...
subroutine FortranCompiler() bind(C, name="compile")
    ...
    do lineNum = 1, size(codeLines), 1
        ...
        if (startswith(errorText, "Invalid") .EQV. .TRUE.) cycle
        if (errorText /= "No Error") exit

    end do
    if (errorText/="No Error" .OR. startswith(errorText, "Invalid") .EQV. .FALSE.) then
