...
module compiler
                ...
                do num = 1, size(tempList), 1
                    ...
                    if (startswith(temp, "brand") .EQV. .TRUE.) then
                        ...
                        if (temp2(1:1)==achar(34)) then
                            ...
                            brandName = "<img class='img-fluid' target='_blank' src='"//trim(getSecondPartAfterEQ(temp2))//"'>"
                        end if
    ...
    function animPartMaker(animpart, numb, img, blur, sat) result(string)
        character(:), allocatable :: animpart, string, numb
        character(4) :: sat
        character(1) :: blur
        character(10000) :: img
...
module fresh
    ...
    function getTheLastChar(text) result(lastChar)
        ...
        do num=len_trim(text), 1, -1
            ...
            & .OR. ichar(text(num:num))==35 .OR. ichar(text(num:num))==59) then
...
subroutine FortranCompiler() bind(C, name="compile")
    ...
    if (errorText/="No Error" .AND. startswith(errorText, "Invalid Command") .EQV. .FALSE.) then
